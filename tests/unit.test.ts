...
function resetMocks(): void {
  ...
  MockedClient.mockImplementation(function MockISPConfigClient() {
    return {
      call: mockCall,
      logout: mockLogout,
...
describe("isp_provision_site (mock-based)", () => {
  ...
  test("full provision with mail and db", async () => {
    ...
    const result = await tool.run({
      ...
      createMail: true,
      createDb: true,
      infoMailboxPassword: "test-info-password",
      adminMailboxPassword: "test-admin-password",
  ...
  test("requires provisioning secrets before making a remote call", async () => {
    const tool = findTool(tools, "isp_provision_site");
    ...
    await expect(tool.run({
      ...
      createMail: true,
      createDb: true,
    }, ctx())).rejects.toThrow("infoMailboxPassword and adminMailboxPassword are required");
...
describe("guard enforcement", () => {
  ...
  const WRITE_TOOLS_WITH_PARAMS: Array<[string, JsonMap]> = [
    ...
    ["isp_ftp_user_add", {}],
    ["isp_cron_add", {}],
    ["isp_client_update", { client_id: 1, params: {} }],
    ["isp_client_delete", { client_id: 1 }],
    ["isp_site_update", { client_id: 1, primary_id: 1, params: {} }],
    ["isp_site_delete", { primary_id: 1 }],
    ["isp_dns_record_update", { type: "A", primary_id: 1, params: {} }],
