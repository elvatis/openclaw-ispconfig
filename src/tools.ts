    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          infoMailboxPassword: { type: "string", description: "Initial password for info@ mailbox. Required when createMail is true.", secret: true },
          adminMailboxPassword: { type: "string", description: "Initial password for admin@ mailbox. Required when createMail is true.", secret: true },
          databasePassword: { type: "string", description: "Initial database-user password. Required when createDb is true.", secret: true },
      ...
      run: async (params, context) => withClient(context, "isp_provision_site", async (client) => {
        ...
        const infoMailboxPassword = String(params.infoMailboxPassword ?? "");
        const adminMailboxPassword = String(params.adminMailboxPassword ?? "");
        const databasePassword = String(params.databasePassword ?? "");
        ...
        if (createMail && (!infoMailboxPassword || !adminMailboxPassword)) {
          throw new Error("infoMailboxPassword and adminMailboxPassword are required when createMail=true");
        }
        if (createDb && !databasePassword) {
          throw new Error("databasePassword is required when createDb=true");
        }
        ...
        if (createMail) {
          ...
          await client.call("mail_user_add", { client_id: newClientId, params: { server_id: serverId, login: `info@${domain}`, email: `info@${domain}`, password: infoMailboxPassword } });
          await client.call("mail_user_add", { client_id: newClientId, params: { server_id: serverId, login: `admin@${domain}`, email: `admin@${domain}`, password: adminMailboxPassword } });
        }

        if (createDb) {
          const dbUserId = await client.call<number>("sites_database_user_add", {
            client_id: newClientId,
            params: { server_id: serverId, database_user: `u${newClientId}_${domain.replace(/\W+/g, "").slice(0, 10)}`, database_password: databasePassword },
          });
